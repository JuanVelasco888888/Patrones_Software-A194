...
def simulate_electrical_metrics(hour: int, demand_kw: float, renewable_ratio: float) -> dict:
    """
    Calcula voltaje, corriente, factor de potencia y pérdida técnica
    estimada para una hora dada.

    renewable_ratio (0.0 - 1.0): qué proporción de la generación de esa
    hora vino de fuentes renovables. Una proporción alta incrementa el
    ruido en el voltaje y el factor de potencia, simulando la variabilidad
    que introducen los inversores de paneles solares y turbinas eólicas
    frente a una generación convencional más estable.
    """
